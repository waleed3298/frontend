			...
			<div className="msgCont">
				<div className="msgBox">
					<div className="msgDisplay">
						{chatLoading ? (
							...
							messages.map(({uuid, recieved, message}) => (
								<Card
									key={uuid}
									className={`mt-3 ${!recieved && 'ml-auto'} mr-2`}
									body
									style={{overflowWrap: 'anywhere',borderRadius:'550px'}}
									bg={recieved ? 'light' : 'success'}
					...
					<Form onSubmit={sendNewMsg}>
						...
						<Button className="btn-block btn-success" type="submit" disabled={!newMsg || newMsg.replace(/\s+/g, '') === ''}>
							Send
						</Button>
					</Form>
				</div>
